...
set(alb_WEB_DIR "${CMAKE_SOURCE_DIR}/web")
set(alb_COI_WORKER "${alb_WEB_DIR}/coi-serviceworker.min.js")
set(alb_SHELL_TEMPLATE "${alb_WEB_DIR}/shell_template.html.in")
set(alb_INDEX_TEMPLATE "${alb_WEB_DIR}/index_template.html.in")
...
if(EMSCRIPTEN)
  ...
  set(alb_WASM_INITIAL_MEMORY "33554432" CACHE STRING
      "Initial WASM memory in bytes (default: 32MB)")
  set(alb_WASM_PTHREAD_POOL_SIZE "4" CACHE STRING
      "Pthread pool size for WASM builds (default: 4)")
  set(alb_WASM_ASYNCIFY_STACK_SIZE "65536" CACHE STRING
...
function(alb_configure_wasm_target target)
  if(EMSCRIPTEN)
    ...
    set_property(GLOBAL APPEND PROPERTY alb_WASM_TARGETS "${target}")
    set_property(GLOBAL PROPERTY alb_WASM_TARGET_${target}_TITLE "${WASM_TITLE}")
    set_property(GLOBAL PROPERTY alb_WASM_TARGET_${target}_DESCRIPTION "${WASM_DESCRIPTION}")
    set_property(GLOBAL PROPERTY alb_WASM_TARGET_${target}_OUTPUT_NAME "${OUTPUT_NAME}")

    target_compile_definitions(${target} PRIVATE alb_WASM_BUILD=1)
    ...
    target_link_options(${target} PRIVATE
      ...
      "-sUSE_PTHREADS=1"
      "-sPROXY_TO_PTHREAD=1"
      "-sPTHREAD_POOL_SIZE=${alb_WASM_PTHREAD_POOL_SIZE}"
      # Enable asyncify for emscripten_sleep and async operations
      "-sASYNCIFY=1"
      "-sASYNCIFY_STACK_SIZE=${alb_WASM_ASYNCIFY_STACK_SIZE}"
      # Memory configuration
      "-sALLOW_MEMORY_GROWTH=1"
      "-sINITIAL_MEMORY=${alb_WASM_INITIAL_MEMORY}"
    ...
    if(EXISTS "${alb_SHELL_TEMPLATE}")
      ...
      message(FATAL_ERROR "Shell template not found: ${alb_SHELL_TEMPLATE}")
    endif()

    # Copy service worker to target build directory for standalone target builds
    if(EXISTS "${alb_COI_WORKER}")
...
function(alb_create_web_dist)
  ...
  get_property(WASM_TARGETS GLOBAL PROPERTY alb_WASM_TARGETS)
  ...
  foreach(target ${WASM_TARGETS})
    get_property(TITLE GLOBAL PROPERTY alb_WASM_TARGET_${target}_TITLE)
    get_property(DESCRIPTION GLOBAL PROPERTY alb_WASM_TARGET_${target}_DESCRIPTION)
...
")
  ...
  if(EXISTS "${alb_INDEX_TEMPLATE}")
    configure_file("${alb_INDEX_TEMPLATE}" "${INDEX_OUTPUT}" @ONLY)
  else()
    message(WARNING "Index template not found: ${alb_INDEX_TEMPLATE}")
  endif()
  ...
  foreach(target ${WASM_TARGETS})
    ...
    get_property(OUTPUT_NAME GLOBAL PROPERTY alb_WASM_TARGET_${target}_OUTPUT_NAME)
